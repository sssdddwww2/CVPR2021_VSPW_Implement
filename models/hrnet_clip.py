...
class TimeAtten(nn.Module):
    def __init__(self,in_dim,out_dim):
        super(TimeAtten,self).__init__()
        self.avgpool = nn.AdaptiveAvgPool2d((1,1))
        self.conv1 = nn.Conv2d(in_dim*2,out_dim,1,1)

    def forward(self,c_img_emb,clip_imgs_emb,clip_num):
        query = self.avgpool(c_img_emb)
        query = query.squeeze(-1).squeeze(-1)
        query = F.normalize(query,dim=1)

        keys = []
        for clip_img_emb in clip_imgs_emb:
            key = self.avgpool(clip_img_emb)
        ...
        ws = torch.cat(ws,1)
        ws = F.softmax(ws,1)
        clip_imgs_emb = [x.unsqueeze(-1) for x in  clip_imgs_emb]
        clip_embs_cat = torch.cat(clip_imgs_emb,-1)
        ws = ws.unsqueeze(1).unsqueeze(1).unsqueeze(1)
        ws = ws.expand_as(clip_embs_cat)
        clip_embs_cat =clip_embs_cat*ws
        clip_embs_cat = torch.mean(clip_embs_cat,-1)
        emb = torch.cat((c_img_emb,clip_embs_cat),1)
        out = self.conv1(emb)
        return out
...
class HRNetV2_clip(nn.Module):
    def __init__(self, n_class,args, **kwargs):
        super(HRNetV2_clip, self).__init__()
        extra = {
        ...
        if self.args.clip_up:

            self.time_atten=TimeAtten(720,720)
        else:

            self.time_atten=TimeAtten(256,256)
    ...
    def forward(self, c_img,clip_imgs, return_feature_maps=False,is_train=True):
        ...
        if self.args.clip_middle:
            ...
            x = self.relu(x)
            x = self.layer1(x)
            x = self.time_atten(x,input2,clip_num)
...
def hrnetv2_clip(pretrained=False, **kwargs):
    model = HRNetV2_clip(n_class=1000, **kwargs)
#    if pretrained:
